...
constexpr auto getCrcItem(std::size_t i) {
    if constexpr (_Bits == 64) {
        constexpr const uint64_t table[256] = {
    ...
    if constexpr (_Bits == 32) {
        constexpr const uint32_t table[256] = {
            UINT32_C(0), UINT32_C(0x77073096), UINT32_C(0xEE0E612C), UINT32_C(0x990951BA),
    ...
    static_assert(_Bits == 32 || _Bits == 64, "Invalid bit count");
}

template <uint32_t _Bits>
constexpr auto calcCrcHash(const char* str, std::size_t size) {
    decltype(getCrcItem<_Bits>(0)) crc = 0;
    crc = ~crc;
    for (std::size_t i = 0; i < size; ++i) {
        const uint8_t byte = str[i];
        crc = getCrcItem< _Bits>(static_cast<uint8_t>(crc) ^ byte) ^ (crc >> 8);
    }
    return crc;
}

constexpr uint64_t operator "" _module(const char* str, const std::size_t size) {
    return calcCrcHash<64>(str, size);
}
